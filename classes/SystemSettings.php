...
class SystemSettings extends DBConnection{
	...
	function update_settings_info(){
		...
		if(isset($_FILES['website-logo']) && $_FILES['website-logo']['tmp_name'] != ''){
			$fname = 'uploads/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['website-logo']['name'];
			$move = move_uploaded_file($_FILES['website-logo']['tmp_name'],'../'. $fname);
			if(isset($_SESSION['system_info']['website-logo'])){
				$qry = $this->conn->query("UPDATE system_info set meta_value = '{$fname}' where meta_field = 'website-logo' ");
				if(is_file('../'.$_SESSION['system_info']['website-logo'])) unlink('../'.$_SESSION['system_info']['website-logo']);
			}else{
				$qry = $this->conn->query("INSERT into system_info set meta_value = '{$fname}',meta_field = 'website-logo' ");
			}
		}
